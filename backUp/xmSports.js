...
Surge
[Script]
小米运动 = type=cron,cronexp="15 17 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js
小米运动获取Token = type=http-response,pattern=^https:\/\/account\.huami\.com\/v2\/client\/login, requires-body=1, max-size=0, script-path=https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js
圈X
[task_local]
# 小米运动
15 17 * * * https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js, tag=小米运动, img-url=https://raw.githubusercontent.com/58xinian/icon/master/xmyd.png, enabled=true
[rewrite_local]
# 小米运动获取Token
^https:\/\/account\.huami\.com\/v2\/client\/login url script-response-body https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js
Loon
[Script]
cron "15 17 * * *" script-path=https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js, tag=小米运动
http-response ^https:\/\/account\.huami\.com\/v2\/client\/login script-path=https://raw.githubusercontent.com/kukudemajia/doudou/master/backUp/xmSports.js, requires-body=true, timeout=3600, tag=小米运动获取Token
 */
  ...
  if (login_token) {
    ...
    if ($.tokenInfo && $.tokenInfo.result === 'ok') {
      ...
      if ($.changeStepRes && $.changeStepRes.code === 1) {
        ...
        $.msg($.name, `${step}步修改成功`, `时间：${timeFormat(localtime())}`, { "open-url": "alipays://platformapi/startapp?appId=20000869" })
      } else {
